...
<script>
    ...
    layui.use(['laydate', 'layer', 'table', 'element'], function () {
        ...
        table.render({
            elem: '#ToBePaid'
            , height: 600
            , url: '${ctx}/order?method=getOrderByStapendDeli'
            , cols: [[
                // {field: 'id', width: 80, title: 'ID', sort: true, fixed: 'left'},
                {field: 'order_no', width: 200, title: '订单号',align:'center', templet: '#idTpl'},
                {field: 'transaction_no', width: 200, title: '订单商户号',align:'center'}
                , {field: 'status', width: 100, title: '状态',align:'center', sort: true, templet: '#statusTpl'}
                , {field: 'order_type', width: 80, title: '类型',align:'center', templet: '#order_typeTpl'}
                ...
                , {field: 'logistics_numbers', width: 200, title: '运单编号',align:'center', sort: true, templet: '#logistics_numbersTpl'}
                ...
                , {field: 'phone', width: 130, title: '买家手机号',align:'center', sort: true, templet: '#phoneTpl'}
                , {field: 'score', width: 300, title: '操作', align:'center',sort: true, toolbar: "#barDemo"}
            ]]
            , id: 'testReload'
            , page: true
            , limit: 100
    ...
    table.on('tool(ToBePaid)', function (obj) {
        var data = obj.data;
        if (obj.event === 'agree') {
            agree(data, obj.event)
        } else if (obj.event === 'refuse') {
            refuse(data, obj.event)
        }
    });
    ...
    function agree(id) {
        layer.open({
            ...
            shade: 0.8,
            area: ['500px', '50%'],
            content: ['LogisticsSubmission.jsp', 'no'],
            yes: function (id) {
            ...
            , success: function (layero, index) {
                $.ajax({
                    ...
                    cache: true,// 不使用ajax缓存
                    contentType: "application/json",
                    url: "${ctx}/order?method=getOrderById&jsonString='+JSON.stringify($('form').serializeObject())",
                    data: {'id': id},   //批量处理 status=2 移入回收站
                    dataType: "json",
                    success: function (data) {
                        var body = layer.getChildFrame('body', index); //巧妙的地方在这里哦
                        if (data.success == 1) {
                            var array = data.rs;
                            for (var obj in array) {
                                body.contents().find("#order_id").val(array[obj].id);
    ...
    function refuse(id) {
        layer.confirm('是否发货？确认后将无法取消', {
            ...
            $.ajax({
                ...
                cache: true,// 不使用ajax缓存
                contentType: "application/json",
                url: "${ctx}/order?method=updateStatus",
                data: {'id': id},
                dataType: "json",
                success: function (data) {
                    if (data.success) {
...
<div class="layui-body">
    ...
    <div style="padding: 15px;">
        <div class="layui-row">
            ...
            <table class="layui-table" id="ToBePaid" lay-filter="ToBePaid"></table>
        </div>
    </div>
