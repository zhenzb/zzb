...
<script>
    ...
    layui.use(['laydate', 'layer', 'table', 'element'], function () {
        ...
        table.render({
            elem: '#ToBePaid'
            , height: 600
            , url: '${ctx}/order?method=getOrderByStatusComp'
            , cols: [[
                // {field: 'id', width: 80, title: 'ID', sort: true, fixed: 'left'},
                {field: 'order_no', width: 210, title: '订单号',align:'center', templet: '#idTpl'},
                {field: 'transaction_no', width: 210, title: '订单商户号',align:'center'}
                , {field: 'status', width: 120, title: '状态',align:'center', sort: true, templet: '#statusTpl'}
                , {field: 'order_type', width: 100, title: '类型',align:'center', templet: '#order_typeTpl'}
                //, {field: 'sign', width: 100, title: '发货仓库', templet: '#signTpl'}
                ...
                , {field: 'nick_name', width: 180, title: '买家昵称', align:'center',templet: '#nick_nameTpl'}
                , {field: 'phone', width: 155, title: '买家手机号',align:'center', sort: true, templet: '#phoneTpl'}
                , {field: 'score', width: 180, title: '操作', align:'center',sort: true, toolbar: "#barDemo"}
            ]]
            , id: 'testReload'
            , page: true
            , limit: 100
    ...
    function Foo(order_no, id ,logistics_numbers) {
       // alert(" order_no   "+order_no + "  id   "+"  logistice_number  "+logistics_numbers);
        layer.open({
            type: 2,
            title: '订单详情',
            shadeClose: true,
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status == 3) {
                           ...
                           // body.contents().find("#orderLogisticsInfo").text(html);
                        }
                        else {
...
<script type="text/html" id="barDemo">
    <a class="layui-btn"
       ...
       lay-event="Foo" onclick="Foo('{{d.order_no}}','{{d.id}}','{{d.logistics_numbers}}')">订单详情</a>
</script>
