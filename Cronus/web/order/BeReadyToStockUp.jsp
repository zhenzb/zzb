...
<script>
    ...
    layui.use(['laydate', 'layer', 'table', 'element'], function () {
        ...
        table.render({
            elem: '#ToBePaid'
            , height: 600
            , url: '${ctx}/order?method=getOrderByStatusStockUp'
            , cols: [[
                // {field: 'id', width: 80, title: 'ID', sort: true, fixed: 'left'},
                {field: 'order_no', width: 240, title: '订单号',align:'center', templet: '#idTpl'},
                {field: 'transaction_no', width: 240, title: '订单商户号',align:'center'}
                , {field: 'status', width: 140, title: '状态',align:'center', sort: true, templet: '#statusTpl'}
                , {field: 'order_type', width: 140, title: '类型',align:'center', templet: '#order_typeTpl'}
                // , {field: 'sign', width: 130, title: '发货仓库', templet: '#signTpl'}
                , {field: 'sku_name', width: 340, title: '商品名称',align:'center', templet: '#skuame'}
                , {field: 'attribute_value', width: 240, title: '规格名称1',align:'center', sort: true, templet: '#attribute_value'}
                , {field: 'attribute_value2', width: 240, title: '规格名称2',align:'center', sort: true, templet: '#attribute_value2'}
                , {field: 'sku_num', width: 240, title: '商品数量',align:'center', sort: true, templet: '#skuNum'}
                , {field: 'created_date', width: 240, title: '下单时间',align:'center', sort: true, templet: '#create_timeTpl'}
                , {field: 'nick_name', width: 200, title: '买家昵称',align:'center', templet: '#nick_nameTpl'}
                , {field: 'phone', width: 160, title: '买家手机号',align:'center', sort: true, templet: '#phoneTpl'}
                , {field: 'score', width: 250, title: '操作',align:'center', sort: true, toolbar: "#barDemo"}
            ]]
            , id: 'testReload'
            , page: true
            , limit: 100
    ...
    function agree(id) {
        layer.open({
            ...
            shade: 0.8,
            area: ['500px', '50%'],
            content: ['StockUp.jsp', 'no'],
            yes: function (id) {

            }
            , success: function (layero, index) {
                var body = layer.getChildFrame('body', index); //巧妙的地方在这里哦
                body.contents().find("#id").val(array[obj].id);
                body.contents().find("#order_no").val(array[obj].order_no);
                body.contents().find("#memo").val(array[obj].memo);
                body.contents().find("#goodsTableOrderCode").text(array[obj].order_no);
            }
        });
...
<script type="text/html" id="barDemo">
    <%--<button lay-event="agree" style="height: 20px;line-height: 20px;width:50px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none">打印</button>
    <button lay-event="refuse" style="height: 20px;line-height: 20px;width:50px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none">开始配货</button>--%>
    <a class="layui-btn"
       style="height: 20px;line-height: 20px;width:50px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none">打印</a>
    <a class="layui-btn"
       style="height: 20px;line-height: 20px;width:100px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none" onclick="agree('{{d.id}}')">开始配货</a>
</script>
...
<div class="layui-body">
    ...
    <div style="padding: 15px;">
        <div class="layui-row">
            ...
            <%--<div class="demoTableReload" style="background-color:#f2f2f2;padding: 15px;">--%>
            <form class="layui-form layui-form-pane">
                <div style="background-color: #f2f2f2;padding:5px 0">
                    ...
                    <div class="layui-form-item" style="margin-bottom:5px">
                        ...
                        <div class="layui-input-inline">
                            <input autocomplete="off" class="layui-input" type="text" name="transaction" id="transaction_no">
                        </div>
            ...
            <table class="" id="ToBePaid" lay-filter="ToBePaid"></table>
        </div>
    </div>
